...
export default function UpdateProfile({ role }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${config.url}/${role}/updateprofile`, formData);
      if (response.status === 200) {
        setMessage(response.data);
        setError('');
        sessionStorage.setItem(role, JSON.stringify(formData)); // Update local session data
      }
    } catch (error) {
      setMessage('');
      if (error.response) {
  ...
  return (
    <div>
      <h3 style={{ textAlign: "center", textDecoration: "underline" }}>Update {role.charAt(0).toUpperCase() + role.slice(1)} Profile</h3>
      {
        message ?
